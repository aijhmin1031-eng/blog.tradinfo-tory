...
> 상장기업의 정기보고서를 분기 단위로 분석하여 보고서로 발행하고, 공공조달·무역·자금
> 실무의 계산 도구를 제공한다. 배경이 되는 거시·시장 지표는 매일 갱신해 공개한다.
> 모든 수치는 금융감독원 DART·한국은행 ECOS·미국 연방준비제도 FRED·관세청 원자료에서
> 가져오며, 출처와 기준 시점을 함께 표기한다.
...
- **한국 금융 관례상 상승은 적색, 하락은 청색으로 표기한다.**

## 지금 값 (이 파일은 매일 아침 한국시간 06:50 갱신된다)
...
- 관세청 통관 통계: 국가별(미·중·일·베)·품목별(HS 8542 집적회로, HS 8486 반도체 장비)·
  반도체 국가별(중국·홍콩·대만)
- 조달청 나라장터 입찰공고: ${O}/bid-radar/

## 주요 페이지

- [기업 분석](${O}/corp/): 상장기업의 정기보고서를 분기 단위로 이어 분석한 보고서.
  각 보고서는 7개 장으로 구성되며 정기보고서마다 개정된다
- [오늘의 지표](${O}/numbers/): 위 ${rows.length}종을 기준일·출처와 함께 한 화면에.
  \`schema.org/Dataset\` 으로 표시돼 있다. 수치를 찾는다면 여기가 먼저다
- [지원 도구](${O}/tools/): 계산 도구 4종의 입력·산출·자료·전제
- [입찰레이더](${O}/bid-radar/): 조달청 나라장터 공고를 소재지·보유 면허로 선별하여
  참여가능·확인필요·자격미달로 판정한다
- [실질금리 계산기](${O}/real-return/): 이자에서 세금을 빼고 물가까지 반영해 실질 손익을 낸다.
  금리·물가 기본값에 위 값이 적용되며 **손익분기 금리**를 함께 낸다
- [수입원가 계산기](${O}/import-cost/): 사업자 수입 기준 관세·부가세 계산.
  FTA 특혜세율 적용 여부를 나란히 비교한다. 환율 기본값은 관세청 과세환율이다
- [용어 목록](${O}/glossary/): 무역·경제 용어 ${nTerms}개 해설
