...
- **Tier A: 39 / 55 완료**(전부 8/25, 계정A) · 대기 16
- **Tier B: 0 / 31 완료** · 대기 31
- **총 39 / 86.** 진행중(`[진행중·*]`) 표시 **없음** = 지금 아무도 잡고 있지 않다. 위에서부터 집으면 된다.
- 완료분 실측: 708~783자 → **1,391~1,814자**(평균 약 1,600자). 기준은 **1,500자 내외**이고
  미달해도 새 사실이 충분하면 통과다(소유주 지시).
- **다음 두 편**: `export-control-grammar`(986자) · `trade-claim-arbitration`(995자).
...
| [진행중·A] | 반도체 수출통제의 문법…뉴스를 구조로 읽는 법 | 986자 | `export-control-grammar` |
| [진행중·A] | 클레임이 걸렸다…무역 분쟁이 법정 대신 중재로 가는 이유 | 995자 | `trade-claim-arbitration` |
| [진행중·A] | 조 단위 화물의 이사…반도체 물류는 왜 초정밀 산업인가 | 1004자 | `semi-precision-logistics` |
| [진행중·A] | 30원짜리 칩이 세운 자동차 공장…차량용 반도체의 교훈 | 1013자 | `car-chip-shortage-lesson` |
