...
- **Tier A: 31 / 55 완료**(전부 8/25, 계정A) · 대기 24
- **Tier B: 0 / 31 완료** · 대기 31
- **총 31 / 86.** 진행중(`[진행중·*]`) 표시 **없음** = 지금 아무도 잡고 있지 않다. 위에서부터 집으면 된다.
- 완료분 실측: 708~783자 → **1,391~1,814자**(평균 약 1,600자). 기준은 **1,500자 내외**이고
  미달해도 새 사실이 충분하면 통과다(소유주 지시).
- **다음 두 편**: `intermediary-trade-types`(923자) · `euv-lithography-intro`(924자).
...
| [진행중·A] | 중계무역·중개무역·가공무역…이름은 닮았지만 돈의 길이 다르다 | 923자 | `intermediary-trade-types` |
| [진행중·A] | 반도체는 빛으로 그린다…노광, 공정의 왕좌 | 924자 | `euv-lithography-intro` |
| [진행중·A] | 세관의 처분에 동의할 수 없을 때…관세 불복의 사다리 | 927자 | `customs-appeal-steps` |
| [진행중·A] | 한국 반도체 지도…기흥에서 용인까지, 클러스터의 지리학 | 929자 | `korea-semi-map` |
