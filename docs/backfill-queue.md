...
- **Tier A: 23 / 55 완료**(전부 8/25, 계정A) · 대기 32
- **Tier B: 0 / 31 완료** · 대기 31
- **총 23 / 86.** 진행중(`[진행중·*]`) 표시 **없음** = 지금 아무도 잡고 있지 않다. 위에서부터 집으면 된다.
- 완료분 실측: 708~783자 → **1,391~1,814자**(평균 약 1,600자). 기준은 **1,500자 내외**이고
  미달해도 새 사실이 충분하면 통과다(소유주 지시).
- **다음 두 편**: `fx-deposit-basics`(864자) · `cpi-how-to-read`(865자).
...
| [진행중·A] | 외화예금 첫걸음…달러 통장에 돈을 두는 이유 | 864자 | `fx-deposit-basics` |
| [진행중·A] | 물가지수 읽는 법…"2% 올랐다"는 문장의 해부 | 865자 | `cpi-how-to-read` |
| [진행중·A] | 무역사기의 전형 다섯 가지…수법은 진화해도 뼈대는 같다 | 871자 | `trade-fraud-patterns` |
| [진행중·A] | 환율은 누가 정하나…서울 외환시장의 하루 | 876자 | `seoul-fx-market-day` |
