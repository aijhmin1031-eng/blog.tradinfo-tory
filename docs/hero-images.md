...
1. **글자·숫자가 들어갔는지** — **대표 이미지(hero)에서는** 들어갔으면 재생성.
   그림 하나가 제목 옆에 놓이는 자리라 깨진 글자가 그대로 눈에 들어온다.
   ★ **다만 「모델이 한글을 못 쓴다」는 이유는 2026-09-02 에 깨졌다.** `openai/gpt-image-2-0`
   은 한글 문장·숫자를 정확히 쓴다(인포그래픽 6장을 글자 하나씩 대조해 확인했다).
   그러므로 **글자 금지는 이제 조판 규칙이지 모델의 한계가 아니다** — 히어로는 글자 없이 가고,
   **글자로 정보를 나르는 그림은 본문 인포그래픽(`src/assets/fig/`)으로 따로 만든다.**
   경계는 하나다. **히어로는 「보는」 그림, 인포그래픽은 「읽는」 그림.**
...
### 기업 분석 `/corp/` 은 다섯째 언어다 — **단면(斷面)** (2026-09-02 신설)

방을 내면서 정했다. 네 언어가 이미 차 있었고(등각=trade · 계기=money · 서류 평면=tariff ·
카툰=basics), 다섯째는 **이 방이 하는 일 그대로** 골랐다 — 갈라서 안을 보이는 것이다.
기사 제목부터 「매출을 **열어 보니**」였다.

```
… 위의 고정 스타일 문장 …
Rendered as a clean vertical cross-section cutaway, a precise engineering section view,
absolutely not isometric.
```

- **`absolutely not isometric` 을 반드시 넣는다.** money·tariff 와 같은 이유로 등각으로
  되돌아가려 한다(문서 「알고 있어야 할 것」).
- **장면은 회사의 겉과 속을 대비시킨다.** 서진시스템 편은 이렇게 썼다 —
  「밀폐 강철 캐비닛을 세로로 갈라 다섯 칸이 드러난다. 맨 위 칸이 전체의 절반 가까이로 가장
  두껍고 그 칸만 번트오렌지다. 지붕 밖으로는 **눈에 띄는 안테나**가 솟아 있고, 거기서 내려온
  가는 선 하나가 아래쪽 **얇은 칸**으로 들어간다. 겉의 안테나와 그것이 먹이는 속의 얇은 칸
  사이의 어긋남이 이 그림의 주제다.」 이름표와 매출 구성의 어긋남이 그대로 장면이 됐다.

### 본문 인포그래픽 — 2026-09-02 에 규약 셋이 바뀌었다 (소유주 지시)

대한광통신 편 일곱 장을 구우면서 소유주가 셋을 잡았다. **셋 다 「AI 티가 난다」는 같은 지적이다.**

1. **★ 왼쪽에 색 세로줄을 대는 인용 블록을 쓰지 말 것.** 생성형 도구의 전형적인 흔적이다.
   마무리 문장은 **가는 가로줄 하나를 긋고 그 아래**에 둔다(신문 조판).
2. **★ 그 자리에 문장만 두지 말고 새 내용을 넣을 것.** 마무리 문장은 본문에 이미 있으므로
   포스터에서는 자리를 낭비한다. 아래 띠는 **두 번째 차트·파생 수치·검증 경로** 중 하나를 든다.
   대한광통신 편에서는 원가율 비교 막대·상반기 누적 매출 막대·자본금 대 자본잉여금 대비 등이 들어갔다.
3. **★ 토리를 넣지 말 것.** 그전에는 넣었으나(서진시스템 편 여섯 장) 반려됐다.
   포스터는 **글자·괘선·표·차트만**으로 구성한다. 서진 편 여섯 장은 소급하지 않는다.

**그리고 포스터마다 구조를 다르게 짠다.** 같은 틀에 숫자만 갈아 끼우면
`docs/corp-analysis.md` 3절이 금지한 「템플릿 대량 생산」이 된다.

### ★ 비율은 「칸을 세게」 하면 정확해진다 (2026-09-02 발견)

인포그래픽의 알려진 한계는 **막대 길이를 정확히 못 그린다**는 것이었다(오차 최대 12%p).
대한광통신 7번 그림은 **39.5% 라는 비율 자체가 결론**이라 그냥 둘 수 없었는데,
「두 뼘 미만으로 그려라」·「중간점 전에 끝내라」로 두 번 고쳐도 계속 52% 로 나왔다.

**해법은 연속량을 이산량으로 바꾸는 것이었다.**
「**열 칸으로 나누고 왼쪽 넉 칸만 채워라**」로 바꾸니 한 번에 정확히 나왔다.
모델은 길이를 못 재지만 **개수는 센다.**
비율이 결론인 그림에는 이 방법을 먼저 쓸 것 — 코드로 옮기기 전에 한 번 해 볼 값어치가 있다.

### ★ 검수 1번을 적용할 때의 판단 하나 (2026-09-02)

이 그림에도 **글자 비슷한 자국이 남았다**(부품 명판 자리에 판독 불가능한 획 몇 개).
100% 로 확대하면 보인다. 그런데 **낱장에서 이 그림이 놓이는 폭은 680px**(모바일 354px)이고
그 크기에서는 질감으로만 남아 글자로 읽히지 않는다. 규칙 1의 취지는
「제목 옆에서 **깨진 글자가 눈에 들어온다**」이므로 여기에 해당하지 않는다고 보고 그대로 썼다.
**다음 사람이 같은 자리에서 망설이지 않도록 적어 둔다 — 판정은 확대본이 아니라 표시 크기에서 한다.**
(비용은 장당 0.97 크레딧 · $0.058 이라 재생성이 아까워서 내린 결정이 아니다.)
...
- ★ **본문 인포그래픽은 히어로와 다른 물건이다**(2026-09-02 신설, 기업 분석 기사에서 갈렸다).
  히어로가 「이 기사가 무엇에 관한 것인가」를 한 장으로 말한다면, 인포그래픽은 **그 절의 수치와
  해석을 전부 담는다.** 그래서 규격도 조판도 다르다 —
  **1800px 4:3 · `src/assets/fig/<슬러그>-<번호>-<이름>.jpg` · `<FigImage>` 로 삽입.**
  세 가지를 지킬 것.
  ① **수치는 손으로 넣어 준다.** 모델은 글자를 정확히 쓰지만 **막대 길이는 정확히 못 그린다**
     (실측 오차가 처음엔 12%p 까지 났고, 프롬프트를 세 번 고쳐 대부분 2%p 안으로 들어왔다).
     그러므로 **비율이 결론인 그림은 코드로 그리고**, 표·구조·문장 중심 그림만 생성에 맡긴다.
  ② **대체 텍스트를 반드시 채운다.** 그림 안의 글자는 검색엔진과 화면낭독기에 존재하지 않는다.
     `check-quality` 가 alt 를 본문 분량에서 걷어 내도록 고쳐 두었으니 길게 써도 된다.
  ③ **원본으로 가는 링크를 남긴다.** 390px 에서는 354px 로 들어가 글자가 4분의 1이 된다
     — `FigImage.astro` 가 원본 링크와 안내 캡션을 자동으로 붙인다.
