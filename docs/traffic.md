...
### 네이버 대조군 실험 — ★ 최종 판정 (2026-08-29): **손대지 않아도 들어온다. 다만 느리다**

**8/26 의 1차 결론(「수집 요청이 있어야 가져간다」)은 틀렸다.** 사흘 뒤 대조군을 다시 재니
**손대지 않은 기사 5편 중 3편이 저절로 색인돼 있었다.**

| 대조군 (IndexNow 만, 수집 요청 없음) | 8/26 | 8/29 |
|---|---|---|
| `semi-export-unit-value` 반도체 수출 단가의 마법 | 안 뜸 | **17건** |
| `cofix-explained` 코픽스 | 안 뜸 | **13건** |
| `fx-hedge-basics` 환헤지 첫걸음 | 안 뜸 | **17건** |
| `tariff-types-explained` 관세의 종류 | 안 뜸 | 0 |
| `dollar-index-explained` 달러인덱스 | 안 뜸 | 0 |

**무엇을 잘못 읽었나.** 8/26 에 대조군이 안 뜬 것을 「수집 요청이 없어서」로 읽었는데,
실제 원인은 **네이버가 느린 것**이었다. 처치군이 30분 만에 뜬 것은 사실이지만, 그것은
**수집 요청이 필수라는 뜻이 아니라 빠르다는 뜻**이었다. **하루는 판정하기에 너무 짧았다.**

**그래서 규약이 바뀐다.**

- **매일 발행분을 손으로 넣을 필요가 없다.** IndexNow 가 CI 에서 매일 아침 자동 제출하고,
  사이트맵·RSS 도 걸려 있다. 아침 점검에 수동 절차를 추가하지 않는다.
- **수집 요청은 의무가 아니라 가속 페달이다.** 안 해도 며칠 안에 들어오고, 하면 30분 만에
  들어온다. **검색 가치가 큰 기사 몇 편에만** 쓴다.
- **남은 문제는 「해야 하는가」가 아니라 「속도」다.** 발행 73편 중 `site:` 로 잡히는 것이
  10건이다. 자동 제출은 되는데 **네이버가 소화하는 속도가 발행 속도를 못 따라온다.**

**교훈**: 하루짜리 관측으로 인과를 결론짓지 말 것. 이 잘못된 결론이 사흘 동안
`traffic.md`·`worklog.md`·`kin-answers.md` 세 문서로 퍼졌다.
...
**→ 판정 결과(8/29): 떴다.** 위 「최종 판정」 절을 볼 것. 이 설계는 이력으로만 남긴다.
...
- **외부 링크 0개.** 색인이 되어도 순위를 올릴 근거가 없다. 지금 가장 큰 빈칸이다.
  **2026-08-29 소유주 결정 「지식인부터 해보자」 — 첫 채널로 네이버 지식인을 연다.**
  절차·규칙·답변 5종은 **`docs/kin-answers.md`** 가 정본이다. 링크 가치가 아니라
  **사람 유입**을 노리는 작업이다(외부 링크의 nofollow 여부는 확인하지 못했다).
  판정 지표는 `visit_referrers` 에 `kin.naver.com` 이 잡히는지다.
