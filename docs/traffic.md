...
- **RSS `https://dotoriecon.com/rss.xml` — 미제출.** 기사 40편을 담고 있고 HTTP 200 이다.
  네이버는 RSS 를 사이트맵과 **별도 채널**로 받으며 **새 글을 가장 빨리 물어 간다.**
  매일 아침 자동 발행 구조와 맞으므로 한 번만 넣으면 이후는 자동이다.

> **사이트맵을 8/24에 받고도 기사가 색인되지 않았다.** 제출을 더 한다고 빨라지는 문제가
> 아니다. 네이버의 새 사이트 웹문서 수집은 느리다. **더 밀지 말고 시간을 줄 것.**
