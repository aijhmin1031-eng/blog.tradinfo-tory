...
## ★ GSC 실적 첫 판독 (2026-09-01) — 노출은 열렸고 클릭은 0이다

**8/26에 「다음 판정 지표는 노출수다」라고 적어 둔 그 값이 나왔다.** 기간 8/22~8/29.

| 검색어 | 클릭 | 노출 | 게재순위 |
|---|---|---|---|
| hbm common api | 0 | 5 | 35.6 |
| **잠정치 뜻** | 0 | 4 | **7.0** |
| 파운드리 | 0 | 4 | 61.5 |
| **관세사 뜻** | 0 | 3 | 11.0 |
| **헤지 뜻** | 0 | 3 | 28.7 |
| 양허세율 협정 법적 근거 | 0 | 2 | 39.5 |
| d ram | 0 | 2 | 90.0 |
| $8.38 | 0 | 1 | 10.0 |
| **만기 뜻** | 0 | 1 | 11.0 |
| **프리미엄 뜻** | 0 | 1 | 12.0 |

*상위 10개(총 18행). 8/22~8/29 클릭 0.*

### 읽어 낸 것 넷

**① 노출은 0에서 벗어났다.** 8/26 기준 「노출 0이면 순위 문제」라고 적어 둔 상태를 지났다.
색인이 실제로 검색 결과로 이어지고 있다.

**② 클릭이 0인 것은 순위 문제만이 아니다.** 「잠정치 뜻」은 **7.0위**인데도 0클릭이다.
1페이지 상단에 있는데 안 눌린다는 뜻이고, 그것은 **질의 자체가 클릭을 부르지 않는 종류**라는
말이다. 「뜻」 검색은 검색 결과 스니펫에서 답이 끝난다(zero-click). 표본이 작아 단정은
못 하지만, 제목·설명을 고쳐서 풀릴 문제가 아닐 가능성이 크다.

**③ 잡힌 질의의 절반이 「뜻」 검색이다.** 잠정치·관세사·헤지·만기·프리미엄 — 다섯 개,
노출 12회. **이것이 2026-08-29 에 「더 쓰지 않기로 한」 바로 그 종류다**(개념·제도 설명,
AI 가 이미 답하는 것). **GSC 가 그 결정을 실측으로 확증했다** — 노출은 되는데 클릭이 없다.

**④ 우리가 팔려는 것으로는 아무것도 안 잡혔다.**
- **도구 질의 0** — 실질금리·수입원가·계산기 관련 검색어가 하나도 없다
- **오늘의 값 질의 0** — 환율·기준금리 관련이 없다
- **브랜드 검색 0** — 「도토리경제」로 찾은 사람이 없다.
  이것은 **외부 채널이 0개**라는 사실의 다른 표현이다(아래 「빈칸」 참조)

### 그래서 무엇을 하나

- **개념 설명 기사를 더 쓰지 않는다**는 결정을 유지한다. 근거가 가설에서 실측으로 바뀌었다.
  기존 편들은 지우지 않는다(신뢰 기반·내부 링크 뼈대, 그리고 노출은 실제로 나온다).
- **도구가 검색에 잡히기까지 기다린다.** `/real-return/` 은 8/30 에 제목을 바꿨고
  `/import-cost/` 도 최근이다. **아직 색인·순위가 붙을 시간이 없었다.** 다음 판독에서
  도구 질의가 나오는지가 판정 지점이다.
- **브랜드 검색 0 을 채우는 것은 검색이 아니라 유통이다.** 2026-09-01 에 붙인
  카카오톡 카드 공유가 그 자리를 노린다.

### 다음 판독

**4주 뒤(2026-09-29 무렵) 같은 화면을 다시 본다.** 볼 것은 셋이다.
① 도구 질의가 나타났는가 ② 브랜드 검색이 0을 벗어났는가 ③ 클릭이 0을 벗어났는가.
**「뜻」 검색의 노출이 늘어나는 것은 성과로 세지 않는다** — 클릭으로 이어지지 않는 것을 확인했다.
...
Search Console 의 **노출수**다. 노출이 0이면 순위 문제다.
★ **노출은 있는데 클릭이 0인 것은 제목·설명 문제로만 읽지 말 것**(2026-09-01 실측으로 고침).
「잠정치 뜻」이 **게재순위 7.0** 인데도 0클릭이었다. 1페이지 상단에서도 안 눌린다면
**질의 자체가 클릭을 부르지 않는 종류**다(「뜻」 검색은 스니펫에서 답이 끝난다).
제목·설명을 손보기 전에 **그 질의가 클릭을 낳는 종류인지** 먼저 본다.
...
## ★ 전략 전환 — 「설명」에서 「데이터·도구」로 (2026-08-29 소유주 제안, 합의)

소유주 제안: **「사람이 검색해서 들어오는 블로그가 아니라, AI가 검색해서 들어오는 블로그를 만들자.」**

**방향은 맞다. 다만 프레임을 하나 바꾸면 더 정확해진다 — 「사람이냐 AI냐」가 아니라
「설명이냐 데이터냐」다.**
...
| | 우리 자산 | AI 시대의 값어치 |
|---|---|---|
| **설명** (HS코드가 무엇인가, FOB가 무엇인가) | 발행 73편 중 **37편** | **없다.** AI 가 학습 데이터로 이미 알고, 사람도 이제 AI 에게 묻는다 |
| **오늘의 값** (기준금리 3.00%, 예금 3.16%) | 시계열 25개 | **있다.** 학습 컷오프가 있어 AI 가 모른다 |
| **우리가 한 계산** (2,464일 대조·세후 실질 -0.11%·11년 10개월 역전) | 자체 산출 | **있다.** 원자료에 없고 우리가 만든 값이다 |
| **도구** (계산기) | `/import-cost/` | **있다.** AI 가 대신 답할 수 없다 — 사용자가 자기 숫자를 넣어야 한다 |

### ★ 함정 하나를 먼저 못 박는다: 인용은 클릭이 아니다

AI 가 우리 숫자를 인용해도 **사용자는 우리 사이트에 오지 않는다.** 답을 이미 받았기 때문이다.
애드센스는 페이지뷰로 도는데 인용은 권위만 쌓고 수익에는 닿지 않는다.

**그래서 「AI 가 들어오는 블로그」를 목표로 삼으면, 트래픽이 안 움직여도 성공처럼 보이는
착시가 생긴다.** 목표는 「AI 가 들어오는 것」이 아니라 **「AI 가 못 하는 것만 하는 것」**이다.
그러면 AI 도 인용하고, 사람도 검색해서 오고(「오늘 환율」은 사람이 검색한다), 도구는 클릭을 만든다.

### 진행 순서 (소유주 승인, 2026-08-29)

**① AI 크롤러가 실제로 오는지 먼저 잰다.**
지금 우리는 **볼 수단이 없다.** 방문 계측이 클라이언트 JS 인데 봇은 JS 를 실행하지 않는다.
`robots.txt` 로 막지 않은 것은 확인했지만(8/29) 그것은 「허용」이지 「온다」가 아니다.
**이것을 모르고 최적화하면 지식인에서 한 실수의 반복이다**(답변을 다 쓰고 나서야 올릴 자리를 셌다).

**② 재고 구성을 설명에서 데이터·환산·갈림으로 옮긴다.** 측정 없이도 옳다.

**③ 싼 것을 얹는다** — `llms.txt`, 시계열 `Dataset` 구조화 데이터, 한글판 「오늘의 숫자」.
하루면 된다. **다만 효과 기대치는 낮게 잡는다** — `llms.txt` 는 관행이지 표준이 아니고
주요 AI 업체가 준수한다는 증거가 약하다.

**④ 도구를 늘린다.** 수익으로 닿는 유일한 길이다.

## 도구가 검색에 잡히게 하기 (2026-08-31)

**도구를 만드는 것과 도구가 발견되는 것은 다른 일이다.** 8/30에 도구 둘을 만들었는데
8/31 점검에서 **기사에서 도구로 가는 링크가 하나도 없었다.** 창고 페이지 한 곳에만 있었다.
검색엔진이 중요도를 판단할 근거가 없고 독자가 도달할 경로도 없다. **도구가 숨어 있었다.**

### 자리 재측정 (2026-08-31)

| 후보 | 1페이지 | 판정 |
|---|---|---|
| 화폐가치·인플레이션 계산기 | 통계청 `mods.go.kr` · NH생명 · 해보셈 · ourcalc · 아는자산 | **레드오션. 만들지 말 것** |
| 수입 관세·관부가세 계산기 | 무역협회 · 트레드링스 · 아이포터 · 포트로직스 · 법제처 | **레드오션. `/import-cost/` 순위는 포기** |
| **실질금리 계산기** | 은행·저축은행중앙회 「예적금 이자 계산기」뿐. **물가를 반영한 도구가 없다** | **여전히 비어 있다. 여기에 집중** |

8/30 실측이 재확인됐다. **명목 이자 계산기는 레드오션, 실질(세금·물가 반영) 계산기는 빈자리다.**

### 한 일

- **제목을 검색어에 맞췄다.** `/real-return/` 제목이 「예금 실질수익 계산기」였는데,
  페이지 주석은 빈자리를 「**실질금리 계산기**」라고 짚고 있었다. **찾아낸 검색어를 제목에
  넣지 않은 것이다.** 제목·h1·JSON-LD `name` 을 「실질금리 계산기」로 바꾸고,
  부제와 `alternateName` 에 「예금 실질수익」을 남겨 그쪽 검색어도 잃지 않게 했다.
- **기사에서 도구로 길을 냈다. 0편 → 4편.**
  `deposit-real-return`(개념 근거 기사) · `deposit-special-rates` ·
  `nominal-vs-real-rate`(9/18) · `deposit-rate-back-to-3`(9/1).
  링크는 끝맺음의 한 항목으로 넣어 문맥에서 자연스럽게 이어지게 했다.
...
- **제목은 우리가 부르고 싶은 이름이 아니라 사람이 검색창에 치는 말로 짓는다.**
  다른 이름은 부제와 `alternateName` 으로 남긴다.
- **레드오션으로 판정된 도구의 순위는 쫓지 않는다.** `/import-cost/` 는 유지하되
  검색 순위를 위해 더 손대지 않는다. 우리 차별점(FTA 적용·미적용 비교)은 기사로 살린다.

## 주제 전환 검토와 결론 (2026-08-31)

소유주가 **「맛집 블로그로 갈아탈까」**를 제안했고, 검토 끝에 **현 주제를 유지**하기로 했다.
**이 검토를 다시 반복하지 말 것.** 근거를 남긴다.

### 데이터는 구할 수 있었다

**행정안전부 「착한가격업소 현황」**(공공데이터포털 `3045247`, CSV, **분기 갱신**, 최신 2026-06-30).
전국 단위이고 업소명·주소·업종·**착한가격 메뉴와 가격**까지 들어 있다. 조달은 문제가 아니었다.

### 그런데 검색 1페이지가 답을 줬다

| 검색어 | 1페이지 | 판정 |
|---|---|---|
| `정읍 맛집` | 식신 · 다이닝코드 · 트립닷컴 · 다음 · 클리앙 · 지역 언론 | **전문 플랫폼.** DB 수만 건, 리뷰 축적, 도메인 10년+ |
| `착한가격업소` | 부산시 · 서울시 물가정보 · 정선군청 · 행안부 · 제주도 · 소비자24 · 파주시 | 민간은 비었지만 **상대가 전부 `go.kr`** |

**맛집은 우리가 무역에서 상대하던 기업 블로그보다 훨씬 센 상대다.**
`착한가격업소` 는 빈자리이긴 하나 **정책 용어라 일반인이 그 말로 검색하지 않는다.**
사람들은 「정읍 맛집」을 친다.

### 광고 단가

금융·무역은 CPC 상위, 음식·지역은 하위라는 것이 업계의 일반적 패턴이다
(**우리 사이트의 실제 단가는 애드센스 승인 후에야 안다. 재서 얻은 값이 아니다**).
같은 수익을 내려면 맛집 쪽은 트래픽이 몇 배 더 필요하다.
**이기기 더 어려운 자리에서 더 많은 트래픽이 필요한 구조다.**

### 결론

**주제를 바꿔도 벽은 그대로다.** 우리 문제는 무역이라는 주제가 아니라 **유통**이다.
색인은 됐고(구글·네이버), 기술은 흠이 없고, 글은 114편이다. **외부 링크가 0개다.**
사이트를 새로 만들면 도메인 나이도 0부터다.

> **수익 = 트래픽 × 클릭률 × CPC.**
> **CPC 가 높아도 트래픽이 0이면 0원이다.** 단가가 높은 자리에 있는 것은 유리한 조건이지
> 그 자체가 수익이 아니다. 이 곱셈을 CPC 한 항으로 읽지 말 것.
...
노출수가 0에서 벗어나면 문이 열린 것이다. **클릭이 0이라고 곧바로 제목·설명을 손보지 않는다** —
2026-09-01 실측에서 7위 질의도 0클릭이었다. 질의의 성격을 먼저 본다(위 「GSC 실적 첫 판독」).
