...
let slugs = args;
// ── --html: 산출 HTML 에서 긴 대시·취소선·서술형 표제를 잡는다 (2026-08-27 신설) ──────
...
if (args.includes('--html')) {
  ...
  for (const f of walk(DIST)) {
    ...
    // ★ 편집 규약 1 — 표제는 명사로 쓴다. 설명하지 않는다(2026-09-07 소유주가 발행분에서 잡았다:
    //    「회사가 스스로를 부르는 이름은 다르다」). 서술형·의문형 표제는 종결어미로 드러나므로
    //    **끝 글자 하나로 갈린다** — 명사구 표제는 「…다」·「…는가」로 끝나지 않는다.
    //    원고와 조판을 가르지 않는다 — 규약은 화면 전체에 걸린다(개편 이후 모든 화면·기사·문서).
    //    넓히기 전에 전수로 재 봤고, 걸린 것은 계산기 두 쪽의 표제 셋뿐이었다(전부 수리).
    for (const m of raw.matchAll(/<h([1234])([^>]*)>([\s\S]*?)<\/h\1>/g)) {
      const t = m[3].replace(/<[^>]*>/g, '').replace(/&[a-z]+;/g, ' ').trim();
      // 두 신호를 함께 본다. ① 종결어미로 끝난다(「…다」·「…는가」)
      // ② 절을 잇는 어미가 가운데 있다(「…비슷한데 순자산은 8.9배」처럼 **명사로 끝나는 문장**은
      //    ①로 안 걸린다. 실제로 발행분에 그런 표제가 있었다).
      const ending = /(?:다|까|는가|ㄴ가|은가|나요)[.?!]?$/.test(t) || /[?]$/.test(t);
      // 연결어미 -ㄴ데/는데 는 「비슷한데」처럼 앞말에 붙어 나오므로 「는데」만 찾으면 못 잡는다.
      // 명사 「가운데」만 예외로 뺀다. 전수로 재 봤고 이 조합의 거짓양성은 0이었다.
      const clause = /(?:지만|더라도|때문에|아니라|이므로|으므로|면서)|(?<!가운)[가-힣]데(?=\s|$)/.test(t);
      if (ending || clause) hits.push({ rel, n: 1, kind: `서술형 표제 「${t}」` });
    }
  }
  if (hits.length === 0) {
  ...
  console.log('  서술형 표제: 발견을 문장으로 늘어놓지 말고 명사구 안에 넣을 것(편집 규약 1).');
