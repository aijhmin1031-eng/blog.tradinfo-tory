...
async function visit(label, ua) {
  ...
  for (const path of ['/', '/forest/']) {
    await page.goto(BASE + path, { waitUntil: 'load' });
    await page.waitForTimeout(900);
  }
  await browser.close();
  console.log(`[${label}] log_visit 호출 ${calls.length}건 ${calls.map((c) => c.p_path).join(' ')}`);
  return calls;
}

const human = await visit('사람 UA', HUMAN);
...
const checks = [
  ...
  ['봇 UA 는 아예 보내지 않는다', bot.length === 0],
];
