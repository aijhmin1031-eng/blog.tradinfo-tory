...
- 데이터 파이프라인(평일 아침 KST 06:50 자동, `npm run pipeline`):
...
- 이미지 생성: Pollo API(nano-banana-2), 캐릭터 일관성은 레퍼런스 이미지 URL 필수(스크래치패드 `tori-char/url_p1.txt`; 유실 시 기존 이미지로 재업로드).
  스타일 프롬프트: "Same flat vector cartoon style as the reference squirrel character (navy vest, round glasses), clean thick outlines, flat solid colors, warm cream background… Strictly no text, no letters, no signboards, no numbers, no watermark."
