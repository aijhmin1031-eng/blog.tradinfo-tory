...
- **기업 분석 방 `/corp/`(2026-09-02 신설)**: 회사마다 **낱장 위키**를 둔다(한눈 카드 + 6장).
  ...
  **강조는 이 항목 하나에만 건다** — 전부 강조하면 강조가 아니다.
  배너 그림은 **도토리 해부도**다(철제 캐비닛 안은 소유주 반려 — 「도토리 컨셉에 안 맞음」).
  겉은 같은 도토리 일곱인데 하나를 갈라 보면 속이 고르지 않다 — 브랜드와 방의 언어(단면)가
  한 그림에서 만난다. **`object-fit` 은 cover 가 아니라 contain 이다**(cover 로 두었더니
  바깥 도토리가 잘려 뜻이 무너졌다). 여백은 그림의 종이색(#FCF6E4)으로 칠해 이음매를 없앴다.
