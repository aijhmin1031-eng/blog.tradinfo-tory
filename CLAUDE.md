...
8. **인용 가능성 3종**(소유주 지시, 2026-08-26): **두괄식 리드**(본문 첫 문단에 결론 수치) ·
   ...
   **★ 2026-08-28부터 이 3종은 분야별로 다르게 걸린다**(소유주 결정 — 「다 똑같은 양식이라 식상하다」).
   ...
   분야별 서술 형식(리포트형·경로 추적형·분기형·문답형)은 `docs/operations.md` 「7. 섹션별 서술 형식」.
   **`check-quality.mjs` 와 `audit.mjs` 가 같은 `FORM` 표를 중복해 들고 있다 — 한쪽만 고치지 말 것.**
...
- **★ `npm run series-check` — 조용히 빈 시계열을 잡는다**(2026-08-28 신설).
  ...
  **`cpi_kr` 의 ECOS 코드는 미검증**이라 첫 성공 실행에서 이 점검기로 확인해야 한다.
